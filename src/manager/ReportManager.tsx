...
interface Report {
  ...
  totalBudget: number;
  status: string;
  location?: string; // Added location field if available or mapped
}
...
const ReportManager: React.FC = () => {
  ...
  if (selectedReport) {
      return (
          <div className="flex-1 flex flex-col min-h-screen bg-[#E6EAF5] pr-5 pt-5">
              <ReportDetailsView 
            ...
            return (
              <div key={report.id} className="bg-white rounded-xl p-6 shadow-sm flex flex-col">
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-semibold ${
                      isCompleted ? "bg-[#107C41] text-white" : "bg-orange-400 text-white"
                    }`}
                  >
                    {report.status}
                  </span>
                ...
                <button 
                   ...
                   className="mt-6 w-full bg-[#1C58D2] hover:bg-blue-700 text-white py-2.5 rounded-md font-semibold text-sm flex items-center justify-center gap-2 transition-colors"
                >
